...
class QuestionClassifier:
    def __init__(self):
        ...
        self.department_qwds = ['什么科', '科室', '哪个科', '哪些科', '可以去', '挂']
    ...
    def classify(self, question):
        ...
        question_entity = medical_dict
        if not medical_dict:
            if 'entity_dict' in globals():
                medical_dict = entity_dict
            else:
                return {}
        ...
        # if检查两个方面：1、疑问词 2、实体
        # 症状
        # 1、疑问词：症状 2、实体：疾病 -> 已知疾病，查症状
        if self.check_words(self.symptom_qwds, question) and ('disease' in types):
            question_type = 'disease_symptom'
            question_types.append(question_type)

        # 1、疑问词：症状 2、实体：具体症状 -> 已知具体症状，查该疾病的其他症状
        if self.check_words(self.symptom_qwds, question) and ('symptom' in types):
            question_type = 'symptom_disease'
            question_types.append(question_type)

        # 原因
        # 1、疑问词：原因 2、实体：疾病 -> 已知疾病，查原因
        if self.check_words(self.cause_qwds, question) and ('disease' in types):
            question_type = 'disease_cause'
            question_types.append(question_type)

        # 并发症
        # 1、疑问词：并发症 2、实体：疾病 -> 已知疾病，查并发症
        if self.check_words(self.acompany_qwds, question) and ('disease' in types):
            question_type = 'disease_acompany'
            question_types.append(question_type)

        # 推荐食品
        # 1、疑问词：食物 2、实体：疾病 3、增强判断：否定词 ->已知疾病与否定词，查食物
        ...
        # 已知食物找疾病
        # 1、疑问词：食物与治疗 2、实体：食物 3、增强判断：否定词 -> 已知食物与否定词，找疾病。
        # （例如：冰糖雪梨可以用于治疗什么疾病？）
        ...
        global intention_dict
        if question_types:
            intention_dict = question_types

        if not question_types:
            if question_entity:
                if 'intention_dict' in globals():
                    question_types = intention_dict

        # 若没有查到相关的疑问词，那么则将该疾病的描述信息返回（前提question中存在实体！！）
        if not question_types and question_entity:
    ...
    def build_wdtype_dict(self):
        ...
        for wd in self.region_words:
            wd_dict[wd] = []            # 字典-键值对，由键获值：键是实体词汇，值是领域
    ...
    def check_medical(self, question):
        ...
        final_dict = {i: self.wdtype_dict.get(i) for i in final_wds}   # 键值对：键是更为详细的实体名称，值是领域

        global entity_dict
        if final_dict:
            entity_dict = final_dict
